...
private class SemanticVisitor(val unit: CompilationUnit) : Visitor<Sequence<SemanticEntity>>() {
    ...
    override fun visitMemberAccessExpr(ctx: MemberAccessExprContext): Sequence<SemanticEntity> {
        ...
        val candidates = visit(ctx.expression())
            .flatMap { expr ->
                when {
        ...
        if (parent is CallExprContext) {
            val actualArgTypes = parent.argument().map { visit(it).mapToType().singleOrNull() ?: AnyType }
            val viable = candidates.filterIsInstance<Executable>().filterOverloads(actualArgTypes)
            return viable.ifEmpty { candidates }
        } else {
            return candidates
        }
    }
    ...
    override fun visitCallExpr(ctx: CallExprContext): Sequence<Type> {
        val callee = visit(ctx.callee).mapToType().filterIsInstance<FunctionType>().singleOrNull()
        return sequenceOf(callee?.returnType ?: AnyType)
    }
